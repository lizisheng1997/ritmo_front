<template>
  <view class="content p35">
    ...
    <view class="fub mt20"
      >{{ t('Verificationcodesentto') }} {{ state.phone }}</view
    >
    <!--  -->
    <view class="form pb40">
      <u-message-input
        ...
        :value="state.code"
        width="120"
        @change="codeChange"></u-message-input>
    </view>
    <view
      class="second"
      :style="{ color: state.counting == true ? '#898784' : '#232322' }"
      @click="
        () => {
      ...
      {{
        state.counting == true
          ? `${state.second}${t('resendinseconds')}`
          : t('Resend')
      }}
    </view>
    <!--  -->
    <view
      class="btn"
      :class="state.code.length == 4 ? '' : 'btnNull'"
      @click="submit"
      >{{ t('Registerandlogin') }}</view
    >
    <view
      class="mt40 fub"
      v-if="state.provider.includes('wxpay')"
      >{{ t('clickingToSend') }}</view
    >
  </view>
</template>
...
const submit = async () => {
  ...
  if (state.provider.includes('wxpay')) {
    uni.login({
      provider: 'weixin',
      onlyAuthorize: true,
